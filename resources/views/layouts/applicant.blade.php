...
<head>
    ...
    <title>@yield('title', 'Dashboard - DEPED Region V Recruitment')</title>
...
<body class="font-sans bg-canvas text-ink">
    <aside class="fixed inset-y-0 left-0 w-[240px] max-md:w-16 bg-surface-card border-r border-hairline px-base py-xl flex flex-col z-[100] transition-all duration-200">
        ...
        <nav class="flex-1">
            <a href="{{ route('applicant.dashboard') }}" class="flex items-center gap-[10px] px-3 py-[10px] text-body rounded-md text-sm font-medium mb-1 no-underline hover:bg-surface-strong hover:text-ink transition-colors {{ Route::is('applicant.dashboard') ? 'bg-primary text-white hover:bg-primary hover:text-white' : '' }}">
            ...
            <a href="{{ route('applicant.jobs') }}" class="flex items-center gap-[10px] px-3 py-[10px] text-body rounded-md text-sm font-medium mb-1 no-underline hover:bg-surface-strong hover:text-ink transition-colors {{ Route::is('applicant.jobs') ? 'bg-primary text-white hover:bg-primary hover:text-white' : '' }}">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="shrink-0"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>
                <span class="max-md:hidden">Job Openings</span>
            </a>
            <a href="{{ route('applicant.profile') }}" class="flex items-center gap-[10px] px-3 py-[10px] text-body rounded-md text-sm font-medium mb-1 no-underline hover:bg-surface-strong hover:text-ink transition-colors {{ Route::is('applicant.profile') ? 'bg-primary text-white hover:bg-primary hover:text-white' : '' }}">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="shrink-0"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>
                <span class="max-md:hidden">Profile</span>
            </a>
        </nav>
        <div class="mt-auto">
            <div class="flex items-center gap-3 p-3 bg-surface-strong rounded-md max-md:hidden">
                <div class="w-10 h-10 bg-primary rounded-full flex items-center justify-center text-white font-semibold text-sm shrink-0">{{ strtoupper(substr(auth()->user()->first_name, 0, 1)) }}{{ strtoupper(substr(auth()->user()->last_name, 0, 1)) }}</div>
                <div class="min-w-0">
                    <div class="font-medium truncate">{{ auth()->user()->first_name }} {{ auth()->user()->last_name }}</div>
                    <div class="text-xs text-body">Applicant</div>
                </div>
            </div>
